    ...
    frappe.call({
        ...
        callback: function (r) {
            if (r.message) {
                ...
                frappe.model.set_value(child.doctype, child.name, "note_view", stripHtml(r.message[0]["note"]).substring(0, 130));
...
frappe.ui.form.on("Quotation", "onload_post_render", function (frm, cdt, cdn) {
    ...
    if(frm.doc.notes && frm.doc.notes.length>=0) {
        cur_frm.doc.notes.forEach(function (note) {
            note.note_view = stripHtml(note.note).substring(0, 130);
        });
